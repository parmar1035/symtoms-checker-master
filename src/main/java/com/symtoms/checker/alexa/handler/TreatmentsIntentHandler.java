...
public class TreatmentsIntentHandler extends AbstractIntentHandler {
	private Logger LOG = LoggerFactory.getLogger(TreatmentsIntentHandler.class);
	...
	protected void handleInternal(HandlerInput input) {
		...
		String healthDetails = healthIssueInfo.TreatmentDescription;
		healthDetails = healthDetails.replaceAll("[^ .,a-zA-Z0-9]", StringUtils.EMPTY);
		addModel(input, "healthDetails", healthDetails);
		symtomsCheckerService.setStepIntoSession(Steps.COMPLETED, input);
